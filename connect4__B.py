...
def drop(*moves, player='Auto', toggle=True, **kws):
    ...
    #Insert it here:
    global B;global isFilled
    if ("reset" in kws):
        B=(["  "]*6,["  "]*6,["  "]*6,["  "]*6,["  "]*6,["  "]*6,["  "]*6)
        n = "X"
        isFilled = (piece[player] == '⚫')
        return True
    token = chr((player == "Auto") and (9898 + isFilled) or ord(piece[player]))
    if(B[0][5]!="  "and B[1][5]!="  "and B[2][5]!="  "and B[3][5]!="  "and B[4][5]!="  "and B[5][5]!="  "and B[6][5]!="  "):
        print(f"The board is full, so {token} cannot randomly place.")
        return False
    c = (len(moves) == 0) and rnd(1,7) or moves[0]
    while(B[c-1][5]!="  "):
        c = (len(moves) == 0) and rnd(1,7) or moves[0]
    row = B[c-1].index("  ")
    B[c-1][row]=token
    ...
    if "announce" in kws : print(f"{token} places into column {c}")
    if (n>1):
        return drop(*moves[1:], player = player, toggle = toggle, **kws)
...
def display():
    ...
    for i in range(6):
        ...
        #        this:  "|⚪|  |⚪|⚪|  |  |⚫|"
        print("|%s|%s|%s|%s|%s|%s|%s|"%(temp[5-i][0],temp[5-i][1],temp[5-i][2],temp[5-i][3],temp[5-i][4],temp[5-i][5],temp[5-i][6]))
        #Line 5:
        if i==5: break
        #Line 6:
        print("├──┼──┼──┼──┼──┼──┼──┤")
